package Master_Cram.Master_Mod.inventory;

import Master_Cram.Master_Mod.item.NewBackpack;
import Master_Cram.Master_Mod.item.NewIcon;
import Master_Cram.Master_Mod.item.NewRing;
import Master_Cram.Master_Mod.player.ExtendedPlayer;
import Master_Cram.Master_Mod.util.NBTHelper;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;
import net.minecraft.entity.player.EntityPlayer;
import net.minecraft.inventory.IInventory;
import net.minecraft.inventory.Slot;
import net.minecraft.item.ItemStack;
import net.minecraft.util.IIcon;

public class SlotBackpack extends Slot {
	
	public EntityPlayer player;
	
	public int slotIndex;
	
	public SlotBackpack(IInventory inventory, EntityPlayer player, int slotIndex, int x, int y)
	{
		super(inventory, slotIndex, x, y);
		this.player = player;
	}

	/**
	 * Check if the stack is a valid item for this slot. Always true beside for the armor slots
	 * (and now also not always true for our custom inventory slots)
	 */
	@Override
	public boolean isItemValid(ItemStack stack)
	{
		// We only want our custom item to be storable in this slot
		if(stack.getItem() instanceof NewBackpack)
		{
			if(stack.hasTagCompound())
			{
				ExtendedPlayer.get(player).backpackInventory.clearInventory();
				ExtendedPlayer.get(player).backpackInventory.readFromNBT(stack.getTagCompound());
			}
			else
			{
				ExtendedPlayer.get(player).backpackInventory.clearInventory();
			}
			
			return true;
		}
		ExtendedPlayer.get(player).backpackInventory.clearInventory();
		return false;
	}
	
	public int getSlotStackLimit()
	{
		return 1;
	}

	@SideOnly(Side.CLIENT)
	public IIcon getBackgroundIconIndex()
	{
		ExtendedPlayer.get(player).backpackInventory.clearInventory();
		return NewIcon.getIcon(2);
	}
}
